...
@bot.tree.command(name="hello", description="this is a test of application commands.")
async def hello(ctx):
    
    await ctx.send("this is a return message from the bot that may or may not work!")

@bot.tree.command(name="code", description="outputs GitHub auth code from tailscale email")
async def code(ctx):
    service = get_service()
    ...
    await ctx.send(f'github code: {code}')

@bot.command() #something from stack overflow, might work for adding commands?
async def sync(ctx: commands.Context):
    await bot.tree.sync()

# leaving this in here while people are actually using the bot
